...
<script>
	$(document).ready(function(){
		...
		$('.delete_task').click(function(){
			...
			_conf("Are you sure to delete this task?", "delete_task", [taskId]);
		});
	});
	...
	function _conf(message, action, params) {
		if (confirm(message)) {
			if (action === "delete_task") {
				delete_task(params[0]); // Call delete_task with the correct ID
			}
		}
	}
		...
		$.ajax({
			...
			success: function(resp) {
				if(resp == 1) {
					...
					alert("Failed to delete the task. Server response: " + resp);
				}
			},
